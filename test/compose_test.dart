...
void main() {
  group("ComposeFn", () {
    test("1", () {
      var result = C(5).x0(addOne).execute();
      expect(result, 6);
    });

    test("2", () {
      var result = C(6).x0(addOne).x0(addOne).execute();
      expect(result, 8);
    });

    test("3 do other data types work", () {
      var result = C(6).x0(addOne).x0(addOne).x0(stringifyIt).execute();
      expect(result, "8");
    });

    test("4 params", () {
      var result = C(6).x0(addOne).x1(addNumber, 3).execute();
      expect(result, 10);
    });
    ...
    test("6 multi params on first", () {
      var result = C(6).x1(addNumber, 2).x1(addNumber, 3).execute();
      expect(result, 11);
    });

    test("7 return function", () {
      var add5andOne = C(5).x0(addOne).execute;
      var result = add5andOne();
      expect(result, 6);
    });

    test("8 with partial application", () {
      var result = C(5).x0((x) => addNumber(x, 1)).execute();
      expect(result, 6);
    });

    test("9 with curry", () {
      var result = C(5).x0(C2(addNumber)(3)).execute();
      expect(result, 8);
    });

    test("10 with curry", () {
      var result = C(5).x0(C2(addOneAndString)("Blah")).execute();
      expect(result, "6Blah");
    });

    // can we do auto currying?
    test("11 with automatic currying", () {
      var result = C(5).c1(stringAndAddOne)("plip").execute();
      expect(result, "6plip");
    });
    ...
    //wouldn't a typed version of this be nice!
    // test("", () {
